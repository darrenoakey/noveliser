...
# Global state for the novel directory
...
def record(step_description: str, previous_result: Any, generator_result: Any, output_dir: Path = Path("output")) -> Any:
    ...
    Args:
        ...
        generator_result: The result from the generator function
    ...
    result_str = _format_for_display(generator_result)
    print(f"{Fore.GREEN}✓ Result: {result_str}{Style.RESET_ALL}")
    
    # Determine the novel directory (stateful)
    novel_dir = _get_or_set_novel_dir(generator_result, output_dir)
    ...
    json_data = _to_json_data(generator_result)
